...
- Search provider는 기본적으로 꺼져 있다. provider 미구성 시 `research_available=false`다.
- 개발 fixture와 외부 Search provider는 동시에 켤 수 없다.
...
| 환경변수 | 기본값 | 허용범위/의미 |
|---|---:|---|
| `PSR_SEARCH_PROVIDER` | `disabled` | `disabled`, `brave` |
...
- 실제 공식 source golden scenario와 사람이 읽는 결과 검토
