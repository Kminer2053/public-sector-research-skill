# PRD — Universal Local Skill v0.1
...
| FR-002 | Government profile 계획 | Must | 법령·정책·조달·개인정보·국제표준 track 생성 |
...
| FR-010 | 보고서 | Must | 동일 `brief.json`에서 한국어 Markdown과 self-contained HTML 생성 |
...
| NFR-006 | 한국어 보고서에서 사실·gap·부분실패를 분리 |
...
1. “공공기관 AI 구매 원칙”을 계획하면 법령·조달·개인정보·데이터권리 track이 생성된다.
2. 공식 HTML, JSON, PDF 또는 로컬 fixture를 입력하면 원문·Passage·hash가 저장된다.
3. Markdown과 HTML 보고서가 citation ID, 원문 위치, score breakdown을 포함한다.
